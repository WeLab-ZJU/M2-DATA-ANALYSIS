bin_ar_20hz = reshape(R_ar,6,[]);
Area_Change_20hz = mean(bin_ar_20hz,1);
% CSDS5797 post environmental affection set frames from 401 to 469 as 0;
% Area_Change_20hz(401:469) = 0;
...
im_20hz=Area_Change_20hz_filter_temp<0.006;
...
for i = 1:length(im_20hz)
    if i<=10
        % head
        status_tem = sum(im_20hz(1,1:(i+10)));
        if status_tem < length(im_20hz(1,1:(i+10)))/2
            im_20hz_smooth(i) = 0;
        else
            im_20hz_smooth(i) = 1;
        end
    else
        if i>(length(im_20hz)-10)
            % tail
            ...
            % intermedian
            status_tem = sum(im_20hz(1,i-10:i+10));
            if status_tem < length(im_20hz(1,i-10:i+10))/2
                im_20hz_smooth(i) = 0;
            else
                im_20hz_smooth(i) = 1;
            end
